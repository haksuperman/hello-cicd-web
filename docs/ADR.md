...
**결정**: JDK 17로 빌드하되 바이트코드는 Java 11 타깃으로 컴파일한다.
**이유**: `tomcat:9.0` 이미지(JDK 11+)와 강의에서 쓰는 Jenkins 서버 JDK(11 이상)에서 모두 동작한다. Spring 5.3은 Java 8~17을 지원한다.
**트레이드오프**: Java 17 언어 기능(record, sealed 등)을 쓸 수 없다.
